        ...
        for path in by_label_paths:
            if os.path.exists(path):
                try:
                    ...
                    logger.warning(f"Error leyendo etiquetas de disco en {path}: {e}")
        return label_map
            ...
            with open(mounts_source, "r") as f:
                for line in f:
                    ...
                    try:
                        ...
                        logger.debug(f"Punto de montaje omitido {mountpoint}: {inner_e}")
                        continue

            return disks

        except Exception as e:
            logger.error(f"Error crítico e inesperado en get_disks: {e}")
            return []
