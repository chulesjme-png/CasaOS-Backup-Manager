...
@patch("app.core.backends.duplicati_backend.DuplicatiClient")
def test_duplicati_backend_executes(mock_client):
    """
    Comprueba que DuplicatiBackend utiliza DuplicatiClient correctamente.
    """
    ...
    configuration = BackendConfiguration(
        ...
        configuration={
            "url": "[http://192.168.1.10:8200](http://192.168.1.10:8200)",
            "timeout": 10,
            "password": "",
        },
    )
    ...
    assert result.metadata["duplicati_version"] == "2.0.8.1"
    assert result.metadata["duplicati_server_state"]["MachineName"] == "Duplicati"

    mock_client.assert_called_once_with(
        base_url="[http://192.168.1.10:8200](http://192.168.1.10:8200)",
        timeout=10,
        password="",
    )
...
@patch("app.core.backends.duplicati_backend.DuplicatiClient")
def test_duplicati_backend_connection_failure(mock_client):
    """
    Comprueba que un fallo de comunicación con Duplicati
    se transforma en un BackupResult fallido.
    """
    mock_client_instance = mock_client.return_value
    mock_client_instance.get_server_state.side_effect = DuplicatiConnectionError(
    ...
    assert "Servidor Duplicati no disponible" in result.errors[0]


@patch("app.core.backends.duplicati_backend.DuplicatiClient")
def test_duplicati_backend_run_backup_success(mock_client):
    """
    ...
    configuration = BackendConfiguration(
        backend_name="duplicati",
        configuration={"url": "[http://192.168.1.10:8200](http://192.168.1.10:8200)"},
    )
    ...
    assert result.execution_reference.resource_id == "42"
    assert result.execution_reference.resource_type == BackupResourceType.TASK
    mock_client_instance.run_backup.assert_called_once_with(1)


@patch("app.core.backends.duplicati_backend.DuplicatiClient")
def test_duplicati_backend_get_status_with_task_id(mock_client):
    """
    Comprueba que GET_STATUS consulta la tarea específica si
    se provee execution_reference con resource_id.
    """
    mock_client_instance = mock_client.return_value
...
@patch("app.core.backends.duplicati_backend.DuplicatiClient")
def test_duplicati_backend_cancel_success(mock_client):
    """
